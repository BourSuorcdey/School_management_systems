...
@section('content')
    <x-forms.post :action="route('admin.classroom.store')">
        @csrf
        <x-backend.card>
            <x-slot name="header">
                @lang('Create Classroom')
            </x-slot>
            ...
            <x-slot name="footer">
                <button class="btn btn-sm btn-primary float-right" type="submit">@lang('Create Classroom')</button>
            </x-slot>
        </x-backend.card>
    </x-forms.post>
@endsection
